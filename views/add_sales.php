<div id="page-wrapper">
	<div class='content'>
		<div class="row">
			<div class="btn-group btn-small" role="group">
				...
				<button class="btn btn-default disabled" style="font-size: 11px; padding: 5px;">Add Sales</button>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-12">
				<h4 class="page-header animated flipInX" style="border: 5px solid #286090;border-top: none;border-bottom: none;border-right: none;padding: 5px;color: #286090;"><span class="fa fa-plus-circle"></span> Add New Sales
				</h4>
			</div>
		</div>
		...
		<div class="col-sm-12" style="border: 1px solid #ccc; padding: 7px; border-radius: 3px">
			<form id="frm_add_dr" action="" method="POST">
				<div class="input-group" style="padding: 4px;">
					...
					<input type="text" class="form-control" readonly required id="dr_number" aria-describedby="basic-addon1" name="delivery_number">
				</div>
				<div class="input-group" style="padding: 4px;">
					<span class="input-group-addon" id="basic-addon1"><strong>Date: <span style="color:red;">*</span></strong></span>
					<input type='text' class="form-control pickadate" name="date" id="date" required>
				</div>
				<div class="input-group" style="padding: 4px;">
				...
				<div class="input-group" style="padding: 4px;">
					...
					<input type='text' class="form-control" name="cashier" value='<?php echo getEncoderName($_SESSION['account_id'])?>' readonly>
				</div>
				<div class="input-group spaceMe" style='margin-left:72%'>
					<div class="btn-group" role="group" style='' aria-label="...">
						<button type="submit" id="btn_add_dr" class="btn btn-sm btn-primary"><span class="fa fa-plus-circle"></span>  Add Sale</button>
					</div>
				</div>
			</form>
		</div>
	</div>
	<div class="col-sm-8" style="padding:10px;display:none;" id="dr_add_details_container">
	  <input type="hidden" id="delivery_id" name="dr_number">
		<div class="col-sm-12" style="border: 1px solid #ccc; padding: 7px; border-radius: 3px;">
		<fieldset class="content-group">
			...
			<div class="col-sm-5" style="padding: 4px;float:left;">
				<div class="input-group">
					<span class="input-group-addon"><strong>Barcode: </strong></span>
					<input type='text' style='text-transform:uppercase' class="form-control" name="barcode" id='barcode' onkeyup='getProduct()' readonly />
				</div>
				
			</div>
			<input style='margin-top: 15px;' type="checkbox" onchange="for_trucking()" id="check_trucking">
			<div class="col-sm-6" style="padding: 4px; float:right;">
				<div class="input-group">
				<span class="input-group-addon"><strong>Item: <span style="color:red;">*</span></strong></span>
					<input type='text' class="form-control" name="product" required id='product' value='' readonly>
				</div>
			</div>
			<div class="col-sm-6" style="padding: 4px; float:right;">
				<div class="input-group">
			...
			<div class="col-sm-6" style="padding: 4px;float:left;">
				<div class="input-group">
					...
					<input type="number" min="0" class="form-control" id="qty" aria-describedby="basic-addon1" name="qty" step="0.01" required/>
				</div>
			</div>
			<div class="col-sm-6" style="padding: 4px;float:right">
				<div class="input-group">
			...
			<div class="col-sm-6" style="padding: 4px;">
				<div class="input-group">
					...
					<input type="number" class="form-control" id="discount" aria-describedby="basic-addon1" name="discount" value='0' step='0.1' required/>
				</div>
			</div>
			</hr>
			<div class="input-group spaceMe" style='margin-left:85%'>
				<div class="btn-group" role="group" aria-label="...">
					<button type="submit" id="btn_add" class="btn btn-sm btn-primary"><span class="fa fa-plus-circle"></span> Add Details</button>
				</div>
			</div>
			</form>
		</fieldset>
	...
	<div class="row"  id="sale_details_container" style="display:none; padding: 10px;">
	  <div class="col-sm-12" style="border: 1px solid #ccc; padding: 7px; border-radius: 3px;">
		<div class="btn-group pull-right" role="group" style="padding: 4px;margin-bottom: 10px;">
			<button type='button' class="btn btn-sm btn-success" onclick='finish_delivery()' id="btn_fin" data-toggle="tooltip" title="Finish Delivery"><span class="fa fa-check-circle"></span> Finish Sales </button>
...
<script>
function getProductScan(id){ // updated 11/16/2017
		$.ajax({
...
function generateDRNum(){
	$.ajax({
		type:"POST",
		url:"ajax/generateSalesNumber.php",
		success:function(data){
			$("#dr_number").val(data);
			$("#delivery_id").val(data);
...
function finish_delivery(){
	...
	$.ajax({
		...
		success: function(data){
			...
			$("#btn_fin").html("<span class='fa fa-check-circle'></span> Finish Sales");
		}
	});
}
...
var myVar = setInterval(function(){ refreshCounter() }, 1000);
function refreshCounter(){
	$.post("ajax/getScannedItem.php", {
	},
	function (data, status) {
		$("#barcode").val(data);
		getProductScan(data);
...
function for_trucking(){
	if($("#check_trucking").prop('checked')){
